angular.module("managerApp")
    .service("CloudProjectCompute", class CloudProjectCompute {
        constructor ($q, OvhApiCloudProjectInstance, OvhApiCloudProjectRegion, OvhApiCloudProjectRegionWorkflowBackup) {
            this.$q = $q;
        ...
        getWorkflowBackup (serviceName, regionName) {
            return this.OvhApiCloudProjectRegionWorkflowBackup.v6().query({ serviceName, regionName }).$promise;
        }
        ...
        getRegionsWithWorkflowService (serviceName) {
            return this.getRegions(serviceName)
                .then(regions => this.$q.allSettled(_.map(regions, region => this.getRegion(serviceName, region))))
                .then(detailedRegions => _.chain(detailedRegions).filter(region => this.isWorkflowServiceUp(region)).map("name").value());
        }

        isWorkflowServiceUp (region) {
